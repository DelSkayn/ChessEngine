...
impl AlphaBeta {
    ...
    pub fn eval_board(&mut self, info: &PositionInfo) -> i32 {
        ...
        if self.gen.is_checkmate(&self.board, info) {
            let color = match self.board.state.player {
                Player::White => -1,
                Player::Black => 1,
            };
            return color * Self::SCORE_CHECKMATE;
        }

        let white_piece_value: i32 = Piece::WhiteQueen
            .to(Piece::WhiteRook)
            .map(|x| self.board.pieces[x].count() as i32 * Self::PIECE_VALUE[x as usize])
            .sum();
        ...
        let white_king_sq = self.board.pieces[Piece::WhiteKing].first_piece();
        let black_king_sq = self.board.pieces[Piece::BlackKing].first_piece();
        ...
        let mut piece_value = white_king_score - black_king_score + white_piece_value
            - black_piece_value
            + (self.board.pieces[Piece::WhitePawn].count() as i32
                - self.board.pieces[Piece::BlackPawn].count() as i32)
                * PAWN_VALUE;

        for p in self.board.pieces[Piece::WhiteBishop].iter() {
            piece_value += Self::BISHOP_TABLE[p.flip()]
        }
        for p in self.board.pieces[Piece::WhiteKnight].iter() {
            piece_value += Self::KNIGHT_TABLE[p.flip()]
        }
        for p in self.board.pieces[Piece::WhiteRook].iter() {
            piece_value += Self::ROOK_TABLE[p.flip()]
        }
        for p in self.board.pieces[Piece::WhitePawn].iter() {
            piece_value += Self::PAWN_TABLE[p.flip()]
        }

        for p in self.board.pieces[Piece::BlackBishop].iter() {
            piece_value -= Self::BISHOP_TABLE[p]
        }
        for p in self.board.pieces[Piece::BlackKnight].iter() {
            piece_value -= Self::KNIGHT_TABLE[p]
        }
        for p in self.board.pieces[Piece::BlackRook].iter() {
            piece_value -= Self::ROOK_TABLE[p]
        }
        for p in self.board.pieces[Piece::BlackPawn].iter() {
