...
impl HashTable {
    ...
    pub fn write(&mut self, entry: Entry, hash: u64, depth: u8, score: TableScore, r#move: Move) {
        ...
        if entry.key != key || score.is_exact() || depth > entry.depth {
    ...
    unsafe fn get_unsafe<'a>(&'a self, hash: u64) -> TableEntry<'a> {
        ...
        for i in 0..6 {
            ...
            if v.key == key && v.r#move != Move::INVALID {
                v.gen_bound = (self.generation as u8) | v.gen_bound & !(Self::GEN_MASK as u8);
            ...
            .min_by_key(|entry| {
                entry.depth as i16
                    - (Self::GEN_CYCLE
                        ...
                        .wrapping_sub(entry.gen_bound as u16)
